...
// Уничтожения скрипера
Game.creeps['Harvester1'].suicide()

/* Важным моментом здесь является то, что память о мертвых крипах не стирается,
а сохраняется для последующего использования.Если вы каждый раз создаете скрипы 
со случайными именами, это может привести к переполнению памяти, поэтому вам 
следует очищать ее в начале каждого тика(перед кодом создания скрипа).*/
...
module.exports.loop = function () {
    ...
    // Бегаем по всем скрипам, тут даже могут быть умершими
    for (var name in Memory.creeps) {
        // если нет скримера с этим именем
        if (!Game.creeps[name]) {
            // то удалить логи связанные с этим скрипером
